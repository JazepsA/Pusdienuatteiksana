...
def dzēst_skolnieku():
    def dzēst_skolnieku_no_db():
        ...
        if id_skolnieka.isdigit():
            ...
            conn.commit()
            messagebox.showinfo("Veiksmīgi", f"Skolēns ar ID {id_skolnieka} tika izdzēsts!")
            logs.destroy()
        else:
            messagebox.showerror("Kļūda", "Lūdzu, ievadiet derīgu ID!")

    logs = tk.Toplevel()
    logs.title("Dzēst skolēnu")
...
def skolnieku_logs():
    ...
    skolnieki_logs.title("Skolēnu pārvaldība")
    skolnieki_logs.geometry(f"300x370+{int((skolnieki_logs.winfo_screenwidth())/2)-150}+{int((skolnieki_logs.winfo_screenheight())/2)-185}")

    pievienot_btn = tk.Button(skolnieki_logs, text="Pievienot skolēnu", command=pievienot_skolnieku, width=25, height=2, bg="lightblue",font="bold",borderwidth=3)
    pievienot_btn.pack(pady=10)

    meklēt_btn = tk.Button(skolnieki_logs, text="Meklēt skolēnu", command=meklēt_skolnieku, width=25, height=2, bg="lightgreen",font="bold",borderwidth=3)
    meklēt_btn.pack(pady=10)

    meklēt_btn = tk.Button(skolnieki_logs, text="Rediģēt skolēna info", command=atjaunot_info, width=25, height=2, bg="DarkGoldenrod1",font="bold",borderwidth=3)
    meklēt_btn.pack(pady=10)

    dzēst_btn = tk.Button(skolnieki_logs, text="Dzēst skolēnu", command=dzēst_skolnieku, width=25, height=2, bg="lightyellow",font="bold",borderwidth=3)
    dzēst_btn.pack(pady=10)
